        ...
        for index, subDir in enumerate(tqdm(sorted(os.listdir(self.root)), desc="Building IMU dataset")):
            if 'imu_' in subDir or 'val_' in subDir or 'test_' in subDir:
                ...
                if  not Path(self.root + 'imuExtracted_data_' + str(self.trim_size) + '.pt').is_file():
                    ...
                    self.temp = np.zeros((self.frame_count*4 - self.trim_size*4, 6))
                    self.temp[:,0] = self.imu_arr_acc[tuple([np.arange(self.trim_size*2, self.frame_count*4 - self.trim_size*2), [0]])]
                    self.temp[:,1] = self.imu_arr_acc[tuple([np.arange(self.trim_size*2, self.frame_count*4- self.trim_size*2), [1]])]
                    self.temp[:,2] = self.imu_arr_acc[tuple([np.arange(self.trim_size*2, self.frame_count*4- self.trim_size*2), [2]])]
                    self.temp[:,3] = self.imu_arr_gyro[tuple([np.arange(self.trim_size*2, self.frame_count*4- self.trim_size*2), [0]])]
                    self.temp[:,4] = self.imu_arr_gyro[tuple([np.arange(self.trim_size*2, self.frame_count*4- self.trim_size*2), [1]])]
                    self.temp[:,5] = self.imu_arr_gyro[tuple([np.arange(self.trim_size*2, self.frame_count*4- self.trim_size*2), [2]])]
