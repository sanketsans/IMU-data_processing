...
os.chdir('/home/sans/Downloads/gaze_data/')
fig = plt.figure()

nT, oT = 1.9, 1.9
roll, pitch = [], [] ## angle at which head moves vertically(roll)
folder = sys.argv[1]
with open(folder + 'gazedata') as f:
    for jsonObj in f:
...
for data in var.gaze_dataList:
    try:
        if(float(data['timestamp']) > 5.0000 and float(data['timestamp']) < 600.0):
        ...
        var.gaze_data[0].append(1.0)
        var.gaze_data[1].append(1.0)

with open(folder + 'imudata') as f:
    for jsonObj in f:
        studentDict = json.loads(jsonObj)
